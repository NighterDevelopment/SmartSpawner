...
| Task | Location |
|---|---|
| Add or change a command | `commands/` (see `commands/CLAUDE.md`) |
...
- `plugin.debug(String)` for diagnostics, gated on the `debug` config key. Not `System.out`.
- One class per file, package layout mirrors feature boundaries. `hooks/` classes are named after the plugin they wrap.
- Files over 600 lines exist (`SpawnerStackerHandler` 1141, `SpawnerDatabaseHandler` 1002, `ListSubCommand` 927, `SpawnerStorageAction` 916, `SpawnerData` 821). Read the surrounding region before editing rather than pattern-matching on a single method.
