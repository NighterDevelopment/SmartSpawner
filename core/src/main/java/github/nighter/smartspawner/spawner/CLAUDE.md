...
`gui/`, `data/` and the root `CLAUDE.md`.

## Packages

| Package | Responsibility |
|---|---|
| `properties/` | The state itself: `SpawnerData`, `VirtualInventory`, `ItemSignature` |
| `data/` | Registry and persistence (`SpawnerManager`, storage backends). See `data/CLAUDE.md` |
| `gui/` | Every inventory GUI plus live sync. See `gui/CLAUDE.md` |
...
`hooks/CLAUDE.md`. Skipping the check is a protection bypass, not a style issue.
...
- `SpawnerLocationLockManager` guards concurrent operations on the same location and needs `shutdown()`.
