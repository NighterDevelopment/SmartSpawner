...
| Check | Called before |
|---|---|
| `CheckBreakBlock.CanPlayerBreakBlock` | breaking a spawner |
| `CheckStackBlock.CanPlayerPlaceBlock` | stacking or placing |
| `CheckOpenMenu.CanPlayerOpenMenu` | opening any spawner GUI |

Each returns `true` early for `isOp()` or `hasPermission("*")`, then asks every enabled plugin and
fails closed on the first denial. Any new player action on a spawner needs the matching check;
omitting it is a protection bypass.

Two traps in this code:

**Inverted method names.** `SuperiorSkyblock2.canPlayerOpenMenu` and `canPlayerStackBlock` return
`!island.hasPermission(...)`, so they are true when the player is **denied**. That is why the two call
sites use them without a leading `!` while every other plugin is negated. Do not "fix" the missing
`!`, and do not copy that naming for a new hook.

**Coverage is not uniform.** `CheckOpenMenu` and `CheckStackBlock` cover all 15 plugins.
`CheckBreakBlock` covers 11: it omits SuperiorSkyblock2, BentoBox, IridiumSkyblock and RedProtect. If
you touch break protection, know that this asymmetry is present in the current code.
...
2. Add a `hasX` field and a `checkPlugin` block in `IntegrationManager`.
3. Add `hooks/protections/api/X.java` with static `canPlayerBreakClaimBlock` / `canPlayerStackClaimBlock` / `canPlayerOpenMenu` as applicable. Return `true` for allow. Catch the plugin's exceptions locally.
4. Wire it into the relevant `Check*` classes.
5. Add the plugin to `dependencies.server` in **both** `plugin.yml` and `paper-plugin.yml` with `required: false` and `join-classpath: true`.
6. Add it to the `protection_plugins` bStats chart in `SmartSpawner.setupBtatsMetrics()` if it is a claim plugin.
7. Document it on the docs site and in `CHANGELOG.md` (use the `write-docs` skill).

Never import a third-party class into a class that loads unconditionally. Keep the import confined to
`protections/api/X.java`, which is only touched when the flag is true, so a missing plugin cannot
